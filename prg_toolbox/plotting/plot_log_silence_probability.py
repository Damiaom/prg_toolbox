...
def labels_log_silence_probability(data, surrogate_data=None):
    ...
    if surrogate_data is not None:
        surrogate_legend = r"$\beta_{Surrogate} = %.2f \pm %.2f$" % (
    ...
    label_dict = {
        "xlabel": r"$C_{size}$",
        "ylabel": "- Log Silence Probability",
        "legend": legend,
    }
    return label_dict


def draw_plot_log_silence_probability(values, ax, plot_kw=None, fill_kw=None):
    """
...
def plot_log_silence_probability(
    data,
    surrogate_data=None,
    ax=None,
    style_config=None,
    ...
    draw_plot_log_silence_probability(values, ax, plot_kw, fill_kw)
    ...
    if surrogate_data is not None and type(surrogate_data) == type(data):
        ...
        draw_plot_log_silence_probability(values_surrogate, ax, plot_kw, fill_kw)
