...
def plot_prg_cascade(
    ...
    fig, axes = plt.subplots(
        1, n_levels, figsize=(col_width * n_levels, row_height * n_leaves), sharey=False
    )
    if n_levels == 1:
        axes = [axes]
    ...
    fig.suptitle(
        f"Coarse graining timeseries: {2**top_level} neurons", y=1.1, fontsize=20
    )
    ...
    for level in range(1, n_levels):
        for node in levels[level]:
            for child in node.children:
                con = ConnectionPatch(
                    xyA=(time_bins - 1, child.y * spacing),
                    coordsA=axes[child.level].transData,
                    xyB=(0, node.y * spacing),
