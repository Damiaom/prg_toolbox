...
def plot_constellation_steps(
    CG_variables, highest_level=4, neuron_tracked=0, positions=None
):
    """
    ...
    final_idx = next(
        i
        for i, clu in enumerate(CG_variables.CG_cluster_idx[highest_level])
        if neuron_tracked in np.atleast_1d(clu)
    )
    target_neurons = list(
        np.atleast_1d(CG_variables.CG_cluster_idx[highest_level][final_idx])
    )
    ...
    for step in range(1, highest_level + 1):
        for c_idx, clu_neurons in enumerate(CG_variables.CG_cluster_idx[step]):
            ...
            if set(clu_array).issubset(set(target_neurons)):
                ...
                prev_idx1 = next(
                    i
                    for i, c in enumerate(CG_variables.CG_cluster_idx[step - 1])
                    if rep1 in np.atleast_1d(c)
                )
                prev_idx2 = next(
                    i
                    for i, c in enumerate(CG_variables.CG_cluster_idx[step - 1])
                    if rep2 in np.atleast_1d(c)
                )
    ...
    for current_step in range(highest_level + 1):
        ...
        if current_step == highest_level:
            ax.scatter(
                *cluster_centers[(highest_level, final_idx)],
                color="gold",
                marker="*",
                s=600,
                zorder=30,
        ...
        ax.set_title(
            f"Step {current_step}",
            fontsize=16,
            color=colors[current_step] if current_step > 0 else "black",
        )
        ax.axis("off")
