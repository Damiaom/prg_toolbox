...
def set_marchenko_pastur_kwargs(
    palette,
    DEFAULT_LINE_KWARGS,
    DEFAULT_FILL_KWARGS,
    hist_kwargs,
...
def draw_powerlaw_in_distribution(spectrum):
    ...
    pl_y_fit = spectrum.pdf_pl_normalization_constant * (
        (pl_x_fit / tail_start) ** -pdf_exponent
    )
...
def draw_plot_marchenko_pastur(spectrum, ax, hist_kw=None, plot_kw=None, vline_kw=None):
    ...
    min_eig = np.min(
        spectrum.avg_across_windows[-1][spectrum.avg_across_windows[-1] > 1e-6]
    )
    ...
    ax.hist(
        spectrum.avg_across_windows[-1], bins=log_bins, label="_nolegend_", **hist_kw
    )
    ax.plot(
        spectrum.mp_x_fit, spectrum.mp_y_fit, color="grey", linestyle="--", linewidth=2
    )

    pl_x, pl_y = draw_powerlaw_in_distribution(spectrum)
    ax.plot(
        pl_x, pl_y, **{**plot_kw, "marker": "none", "linewidth": 2, "linestyle": "-"}
    )
...
def plot_marchenko_pastur(
    ...
    hist_kw, plot_kw, vline_kw = set_marchenko_pastur_kwargs(
        DEFAULT_LINE_KWARGS, DEFAULT_FILL_KWARGS, hist_kwargs, plot_kwargs, vline_kwargs
    )
...
def labels_covariance_spectrum(data, surrogate_data=None):
    ...
    label_dict = {"xlabel": r"$rank/N$", "ylabel": "Eigenvalue", "legend": legend}
    return label_dict
...
def set_covariance_spectrum_values_and_kwargs(
    DEFAULT_LINE_KWARGS,
    DEFAULT_FILL_KWARGS,
    plot_kwargs,
    fill_kwargs,
    ...
    if data_type_name == "covariance_spectrum":
        ...
        raise ValueError(
            "Data must be either a covariance_spectrum object or a dictionary."
        )

    number_of_colors = len(values["y"])
    colors_by_iteration = set_colors_from_palette(
        number_of_colors, palette, data_or_surrogate=data_or_surrogate
    )
    plot_kw = [
        {**DEFAULT_LINE_KWARGS, "color": colors_by_iteration[k], **(plot_kwargs or {})}
        for k in range(len(values["y"]))
    ]
    fill_kw = [
        {**DEFAULT_FILL_KWARGS, "color": colors_by_iteration[k], **(fill_kwargs or {})}
        for k in range(len(values["y"]))
    ]
...
def draw_plot_covariance_spectrum(values, ax, plot_kw=None, fill_kw=None):
    ...
    log_y0 = np.mean(
        np.log(values["y"][-1][1 : int(len(values["y"][-1]) / 10)])
        - exponent * np.log(values["x"][-1][1 : int(len(values["y"][-1]) / 10)])
    )
    y0 = np.exp(log_y0)
    ax.plot(
        values["x"][-1],
        powerLaw_function(values["x"][-1], y0, exponent),
        color=plot_kw[-1]["color"],
        linestyle="--",
        lw=3,
        alpha=0.8,
    )
...
def plot_covariance_spectrum(
    ...
    values, plot_kw, fill_kw = set_covariance_spectrum_values_and_kwargs(
        DEFAULT_LINE_KWARGS,
        DEFAULT_FILL_KWARGS,
        plot_kwargs,
        fill_kwargs,
    ...
    if surrogate_data is not None and type(surrogate_data) == type(data):
        values_surrogate, plot_kw, fill_kw = set_covariance_spectrum_values_and_kwargs(
            DEFAULT_LINE_KWARGS,
            DEFAULT_FILL_KWARGS,
            plot_kwargs,
            fill_kwargs,
    ...
    if add_marchenko_pastur_inset:
        ...
        if not hasattr(data, "mp_sigma"):
            raise AttributeError("The spectrum object lacks Marchenko-Pastur fit data.")
        ...
        hist_kw, plot_mp_kw, vline_kw = set_marchenko_pastur_kwargs(
            palette,
            DEFAULT_LINE_KWARGS,
            DEFAULT_FILL_KWARGS,
            mp_hist_kwargs,
