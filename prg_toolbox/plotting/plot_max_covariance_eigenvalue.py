...
def labels_max_covariance_eigenvalue(data, surrogate_data=None):
    ...
    legend = (
        [result_legend, surrogate_legend, "Constant"]
        if surrogate_legend
        else [result_legend, "Constant"]
    )
    label_dict = {
        "xlabel": r"$C_{size}$",
...
def draw_plot_max_covariance_eigenvalue(values, ax, plot_kw=None, fill_kw=None):
    ...
    log_y0 = np.mean(
        np.log(values["y"][1:]) - values["exponent"] * np.log(values["x"][1:])
    )
    y0 = np.exp(log_y0)
    ax.plot(
        values["x"][1:],
        powerLaw_function(values["x"][1:], y0, values["exponent"]),
        color=plot_kw["color"],
        linestyle="--",
        lw=3,
        alpha=0.8,
...
def plot_max_covariance_eigenvalue(
    ...
    (
        ...
        COLORS,
    ) = set_default_kwargs(colors)

    # ------------ Results plot ----------------------------- (Merge defaults with user-provided kwargs)

    values, plot_kw, fill_kw = set_values_and_kwargs(
        DEFAULT_LINE_KWARGS,
        DEFAULT_FILL_KWARGS,
        plot_kwargs,
        fill_kwargs,
    ...
    if surrogate_data is not None and type(surrogate_data) == type(data):
        values_surrogate, plot_kw, fill_kw = set_values_and_kwargs(
            DEFAULT_LINE_KWARGS,
            DEFAULT_FILL_KWARGS,
            plot_kwargs,
            fill_kwargs,
