...
def labels_max_covariance_eigenvalue(data, surrogate_data = None):
    """
    Creates axis labels and legend entries for max covariance eigenvalue plots.
    ...
    """
    result_legend = r'$\epsilon = %.2f \pm %.2f$' % (
        data["exponent"],
        data["error"],
    )
    if surrogate_data is not None:
        surrogate_legend = r'$\epsilon_{Surrogate} = %.2f \pm %.2f$' % (
    ...
    legend = [result_legend, surrogate_legend, "Constant"] if surrogate_legend else [result_legend, "Constant"]
    label_dict = {
        "xlabel": r"$C_{size}$",
        "ylabel": "Max Covariance Eigenvalue",
        "legend": legend
    }
    return label_dict

def draw_plot_max_covariance_eigenvalue(values, ax, plot_kw=None, fill_kw=None):
    """
    Draws max covariance eigenvalue data and fitted power-law. 
    ...
    """
    # Main line
    ax.plot(values["x"][1:], values["y"][1:], label="_nolegend_", **plot_kw)

    # Std error bar
    if values["y_low"] is not None and values["y_high"] is not None:
        ax.fill_between(values["x"][1:], values["y_low"][1:], values["y_high"][1:], label="_nolegend_", **fill_kw)

    log_y0 = np.mean(np.log(values["y"][1:]) - values["exponent"] * np.log(values["x"][1:]))
    y0 = np.exp(log_y0)
    ax.plot(values["x"][1:], powerLaw_function(values["x"][1:], y0, values["exponent"]), color=plot_kw["color"],linestyle = '--', lw=3, alpha = 0.8)

def plot_max_covariance_eigenvalue(
    data,
    surrogate_data = None,
    ax=None,
    plot_kwargs=None,
    ...
    """
    Plots max covariance eigenvalue scaling with optional surrogate and reference.
    ...
    Args:
        ...
        fill_kwargs (dict or None)   : Fill (error band) kwargs
    ...
    draw_plot_max_covariance_eigenvalue(values, ax, plot_kw, fill_kw)
    ...
    if surrogate_data is not None and type(surrogate_data) == type(data):
        ...
        draw_plot_max_covariance_eigenvalue(values_surrogate, ax, plot_kw, fill_kw)
    ...
    ax.plot(values["x"], values["y"][1]*np.ones(len(values["x"])), **plot_kw)
    ...
    all_labels = labels_max_covariance_eigenvalue(values, values_surrogate)
