...
def labels_mean_variance(data, surrogate_data=None):
    """
    Creates axis labels and legend entries for mean-variance plots.
    ...
    legend = (
        [result_legend, surrogate_legend, "Independent"]
        if surrogate_legend
        else [result_legend, "Independent"]
    )
    label_dict = {"xlabel": r"$C_{size}$", "ylabel": "Mean Variance", "legend": legend}
    return label_dict


def draw_plot_mean_variance(values, ax, plot_kw=None, fill_kw=None):
    """
    Draws mean-variance data and fitted power-law on an axis.

    Parameters
    ----------
        values (dict)            : Dictionary with x, y, y_low, y_high, exponent, error
    ...
    ax.plot(
        values["x"],
        powerLaw_function(values["x"], y0, values["exponent"]),
        color=plot_kw["color"],
        linestyle="--",
        lw=3,
        alpha=0.8,
...
def plot_mean_variance(
    ...
    draw_plot_mean_variance(values, ax, plot_kw, fill_kw)
    ...
    if surrogate_data is not None and type(surrogate_data) == type(data):
        ...
        draw_plot_mean_variance(values_surrogate, ax, plot_kw, fill_kw)
    ...
    ax.plot(values["x"], values["y"][0] * values["x"], **plot_kw)
