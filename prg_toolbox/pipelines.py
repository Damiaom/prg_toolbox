...
from .config import *
from .verbosity import print_if_full, timed_step
...
def run_PRG(data, user_params: AnalysisParams = None):
    ...
    with timed_step("discard_transient", verbose):
        data = (
            discard_transient(
                ...
                transient_time_ms=discard_transient_time_ms,
            )
    ...
    with timed_step("slice_by_time_window", verbose):
        time_windows_list = (
            slice_by_time_window(
                ...
                timeseries_binsize_ms=binsize,
                overlap_fraction=slice_overlap,
                t_start=0.0,
                verbose=verbose,
            )
    ...
    for slice_idx, current_slice in enumerate(time_windows_list):
        for sample_idx in range(nsamples):
            print_if_full(
                f"time slice {slice_idx + 1}/{n_slices}, sample {sample_idx + 1}/{nsamples}:",
                verbose,
            )
            ...
            if samplesize is not None:
                with timed_step("pick_random_sample", verbose, indent="  "):
                    subsample = pick_random_sample(
                        current_slice,
                        samplesize,
                        data_format=data_format,
                        random_seed=combined_seed,
                    )
            else:
            ...
            with timed_step("binarize_data", verbose, indent="  "):
                binary_time_series = binarize_data(
                    subsample,
                    data_format=data_format,
            ...
            for call in observable_call_list:
                with timed_step(f"observable: {call.__name__}", verbose, indent="  "):
                    ...
                    observable_stacker[call.__name__].append(
                        CG_observable.avg_across_windows
                    )
                if hasattr(CG_observable, "exponent"):
                    print_if_full(
                        f"    {call.__name__}: exponent = {CG_observable.exponent:.4g}",
                        verbose,
                    )
    ...
    for call in observable_call_list:
        ...
        if not is_function_observable(call):
            ...
            CG_observable = call(cgvar)
            CG_observable = average_observable_sample_values(
                CG_observable, stacked_data
            )
        else:
            stacked_data = observable_stacker[key]
            CG_observable = call(cgvar)
            CG_observable = average_observable_sample_values_for_functions(
                CG_observable, stacked_data, rg_steps, binary_time_series
            )
...
def run_PRG_in_directory(
    ...
    if (
        not show_plots
        and not save_results
        and not save_plots
        and prg_params.verbose != "silent"
    ):
        print(
            "Warning: You are not showing or saving any results. Set show_plots or save_plots or save_results to True to see or keep your results."
        )
    ...
    if isinstance(file_directory, (list, tuple)):
        ...
        all_files = [
            os.path.join(file_directory, f) for f in sorted(os.listdir(file_directory))
        ]

    N = len(all_files)
    results_path, plots_path = (
        save_manifest(all_files, prg_params) if save_results else (None, None)
    )
    ...
    for i, path in enumerate(all_files, start=1):
        ...
        result_dict = run_PRG(data=data, user_params=prg_params)

        # Show plots for the observables
        if show_plots or save_plots:
            make_plots_for_observables(
                result_dict, prg_params, show_plots, save_plots, plots_path, file_key
            )
...
def run_PRG_in_directory_parallel(
    file_directory,
    skipped_files_list=None,
    user_params=None,
    show_plots=False,
