...
def _load_timestamps(
    ...
    if format == "tabular":
        ...
        raw_array = (
            _load_npy_maybe_pickled(file_or_path)
            if isinstance(file_or_path, str)
            else file_or_path
        )
...
def _load_timeseries(filepath, delimiter=None, mat_key=None):
    ...
    if ext == ".npy":
        ...
        raise ValueError(
            f"Unsupported file format: '{ext}'. Please use .npy, .mat, .txt, .csv, or .dat."
        )

    if data.ndim != 2:
        raise ValueError(
            f"Expected a 2D matrix, but loaded array has shape {data.shape}. "
...
def load_data(filepath, user_params: AnalysisParams = None):
    ...
    if params.data_format == "timeseries":
        return _load_timeseries(
            filepath, delimiter=params.delimiter, mat_key=params.mat_key
        )
...
def discard_transient(
    data, data_format="timeseries", timeseries_binsize_ms=1, transient_time_ms=0
):
    """
    ...
    if data_format == "timeseries":
        filtered_data = _discard_transient_from_timeseries(
            data, timeseries_binsize_ms, transient_time_ms
        )
    else:
...
def pick_random_sample(data, sample_size, data_format="timeseries", random_seed=123):
    ...
    if data_format == "timeseries":
        ...
        raise ValueError(
            "data_format must be either 'timeseries', 'tabular' or 'numpy_2col'"
        )


def _slice_timestamps(
    timestamps, window_duration_ms, overlap_fraction=0.0, t_start=0, verbose="warnings"
):
    """
...
def slice_by_time_window(
    ...
    if data_format == "timeseries":
        return _slice_timeseries(
            data, window_duration_ms, timeseries_binsize_ms, overlap_fraction, verbose
        )
    elif data_format == "tabular" or data_format == "numpy_2col":
        return _slice_timestamps(
            data, window_duration_ms, overlap_fraction, t_start, verbose
        )
    else:
        raise ValueError(
            "data_format must be either 'timeseries', 'tabular' or 'numpy_2col'"
...
def _shuffle_isi_timestamps(timestamps, random_seed=None):
    ...
    for unit in unique_units:
        ...
        unit_shuffled_array = np.column_stack(
            (shuffled_times, np.full_like(shuffled_times, unit))
        )
...
def shuffle_isi(data, data_format="timeseries", random_seed=None):
    ...
    if data_format == "timeseries":
        ...
        raise ValueError(
            "data_format must be either 'timeseries', 'tabular' or 'numpy2col'."
        )
...
def binarize_data(
    ...
    if data_format != "timeseries":
        ...
        if binary_method == "zscore_threshold":
            ...
            raise ValueError(
                f"Unknown binary_method: '{binary_method}'. "
...
def average_observable_sample_values(CG_observable, stacked_results):
    if (
        type(CG_observable) != obs.max_covariance_eigenvalue
        and type(CG_observable) != obs._avalanche_covariance_eigenvalue
    ):
        ...
        (
            CG_observable.exponent,
            CG_observable.exponent_error,
            CG_observable.exponent_r2,
        ...
        (
            ...
            CG_observable.avg_across_windows[1:], skip_first_value=True
        )
...
def average_observable_sample_values_for_functions(
    CG_observable, stacked_results, rg_steps, raw_timeseries
):
    CG_observable.avg_across_windows, CG_observable.std_across_windows = (
        average_across_windows_for_functions(stacked_results, rg_steps)
    )
    ...
    if type(CG_observable) == obs.covariance_spectrum:
        (
            ...
            CG_observable.avg_across_windows[-1][: CG_observable.fit_length],
            spectrum=True,
        )
        CG_observable.exponent = -CG_observable.exponent
        (
            CG_observable.mp_x_fit,
            CG_observable.mp_y_fit,
            CG_observable.mp_lambda_plus,
            CG_observable.mp_sigma,
            CG_observable.mp_Q,
        ) = CG_observable.get_marchenko_pastur(raw_timeseries)
        CG_observable.pdf_pl_exponent, CG_observable.pdf_pl_normalization_constant = (
            CG_observable.get_distribution_power_law()
        )
...
def make_plots_for_observables(
    result_dict,
    ...
    """
    plot_call_list = {
        "mean_variance": plot.plot_mean_variance,
...
def save_manifest(files, prg_params: AnalysisParams, skipped_files_list=None):
    ...
    hash_params = {
        k: v for k, v in clean_params.items() if k not in ("verbose", "save_path")
    }
...
def save_result_dictionaries(result_dict, prg_params, file_key, analysis_save_path):
    ...
    for observable in prg_params.observables:
        ...
        file_results[name] = {
            ...
            # store arrays
            "avg_across_windows": data_obj.avg_across_windows
            if hasattr(data_obj, "avg_across_windows")
            else None,
            "std_across_windows": data_obj.std_across_windows
            if hasattr(data_obj, "std_across_windows")
            else None,
        }
        if name == "covariance_spectrum":
            ...
            file_results[name]["mp_lambda_plus"] = getattr(
                data_obj, "mp_lambda_plus", None
            )
            file_results[name]["mp_sigma"] = getattr(data_obj, "mp_sigma", None)
            file_results[name]["mp_Q"] = getattr(data_obj, "mp_Q", None)
            file_results[name]["pdf_pl_exponent"] = getattr(
                data_obj, "pdf_pl_exponent", None
            )
            file_results[name]["pdf_pl_normalization_constant"] = getattr(
                data_obj, "pdf_pl_normalization_constant", None
            )
